...
// SUBJECT in gold ("S"), each comp in Crow navy, numbered to match the
// Comparison table and the map pins. Bars are sorted smallest → largest
// (Mason, 9/24/26) — the number label travels with its bar, so it still
// identifies the same deal everywhere — and hovering a bar names the deal.
// Pure SVG, server-rendered.
import { fmtShort } from "./MetricStrip";

export interface BarItem {
  label: string; // "S" for the subject, "1".."N" for comps
  name: string; // the deal's name — shown on hover
  value: number | null;
  isSubject: boolean;
}
...
export default function MetricBars({
  ...
  // Smallest → largest; deals without the metric trail at the end.
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" role="img"
      aria-label={ordered.map((it) => `${it.name}: ${it.value != null ? fmtShort(kind, it.value) : "—"}`).join(", ")}>
      {/* baseline */}
      ...
      {ordered.map((it, i) => {
        ...
        if (it.value == null || !isFinite(it.value)) {
          return (
            <g key={i}>
              <title>{`${it.name}${it.isSubject ? " (Subject)" : ""} — not stated`}</title>
        ...
        return (
          <g key={i}>
            <rect x={xOf(i)} y={y} width={barW} height={h} rx={2}
              ...
              {/* Hover names the deal (Mason, 9/24/26). */}
              <title>{`${it.name}${it.isSubject ? " (Subject)" : ""} — ${fmtShort(kind, it.value)}`}</title>
            </rect>
